...
프로젝트별 기여 범위와 코드 소유 범위를 다음 기준으로 구분했습니다.
...
- 제출한 C# 코드는 지원자가 요구사항을 정의하고 구조를 설계한 뒤 직접 구현·수정·검증한 코드입니다.
- 캐릭터 모델, 애니메이션, 아이콘과 기타 외부 리소스는 코드 소유 범위에 포함하지 않습니다.
- 전체 Unity 프로젝트가 아니라 포트폴리오 검토에 필요한 핵심 코드와 설명 문서를 선별했습니다.


## 1.6 AI 활용 범위

- 생성형 AI를 설계 대안 비교, API 및 구현 방식 검토, 디버깅과 테스트 케이스 점검을 위한 보조 도구로 활용했습니다.
- AI가 제안한 내용은 현재 요구사항과 코드 구조에 맞는지 확인한 뒤 직접 수정·적용했습니다.
- 최종 구조 선택, 예외 조건 정의, 코드 통합과 실행 검증은 지원자가 직접 수행했습니다.
...
| Hey Cheese! | 공동개발 | 미니게임 입력, 조건 판정, 상태 피드백 | 100% |

> 제출 코드 작성 비중 100%는 **본 Repository에 선별해 포함한 파일 기준**입니다. 공동개발 프로젝트 전체 코드 기여율을 의미하지 않으며, 프로젝트 전체 및 개발 파트 기여 비중은 각 프로젝트 표에 별도로 제시했습니다.